 ...
 * @author cke
 */
@DeployableVerticle(isWorker = true) public class IndicatorVerticle extends AbstractGuiceVerticle {
	/**
	 * The Constant GET.
	 */
		...
		vertx.eventBus().registerHandler(GET_BY_CODE, new Handler<Message<String>>() {
			...
			@Override public void handle(final Message<String> message) {
				...
				try {
					utils.testHTTPMetod(Constantes.POST, req.getMethod());
					JsonObject params = new JsonObject(req.getBody());
					utils.testMandatoryParams(params.toMap(), PARAM_ACTIVITY_ID, PARAM_COUNTRY_ID, PARAM_INDICATOR_CODE);
