...
@DeployableVerticle(isWorker = false) public class ShippingVerticle extends AbstractGuiceVerticle {
	...
	@Inject @Named("payplug") private JsonObject config;
			...
			@Override public void handle(final Message<String> message) {
				...
				try {
					...
					HttpClientRequest request = client.post(config.getString("basePath") + "/payments", new Handler<HttpClientResponse>() {
						@Override public void handle(final HttpClientResponse resp) {
