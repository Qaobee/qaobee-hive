...
public class UtilsImpl implements Utils {
	...
	@Inject private HabilitUtils habilitUtils;

	@Override public void sendError(final Message<String> message, final ExceptionCodes ex, final String info) {
		message.fail(ex.getCode(), Json.encode(new QaobeeException(message.body(), ex, info)));
	}

	@Override public void testHTTPMetod(final String allowed, final String tested) throws NoSuchMethodException {
		if (!allowed.equals(tested)) {
			throw new NoSuchMethodException(tested + " is not allowed");
		}
	}

	@Override public Map<String, List<String>> toMap(final MultiMap multiMap) {
	...
	@Override public void sendError(final Message<String> message, final QaobeeException e) {
		message.fail(e.getCode().getCode(), Json.encode(e));
	}

	@Override public void sendError(Message<String> message, ReplyException e) {
	...
	@Override public void sendErrorJ(final Message<JsonObject> message, final ExceptionCodes code, final String error) {
		message.fail(code.getCode(), error);
	}

	@Override public void saveAndResizeImage(final File source, final String dest, final int width, final int height, final boolean isSquare) throws IOException {
	...
	@Override public String formatDate(final long timestamp, final int dateStyle, final int timeStyle, final String locale) {
	...
	@Override public JsonObject find(final String key, final String value, final JsonArray res) {
	...
	@Override public void sendStatus(final boolean b, final Message<String> message) {
	...
	@Override public void sendStatusJson(final boolean b, final Message<JsonObject> message) {
		final JsonObject jsonResp = new JsonObject();
		jsonResp.putBoolean("status", b);
	...
	@Override public void testMandatoryParams(Map<String, ?> map, String... fields) {
		...
		for (final String field : fields) {
			...
			} else if ((map.get(field) instanceof List && ((((List<?>) map.get(field)).isEmpty()) || (StringUtils.isBlank((String) ((List<?>) map.get(field)).get(0))))) || (
	...
	@Override public void testMandatoryParams(String body, final String... fields) {
		if (null == body) {
			body = "{}";
		}
	...
	@Override public User isUserLogged(RequestWrapper request) throws QaobeeException {
		String token = "";
	...
	@Override public User isLoggedAndAdmin(RequestWrapper request) throws QaobeeException {
	...
	@Override public long randomDate(int yearOldMin, int yearOldMax) {
